...
def create_voting_dict(strlist):
    ...
    for row in strlist:
        cols = row.split()
        vd[cols[0]] = [int(x) for x in cols[3:]]
    return vd
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    for i in range(len(voting_dict[sen_a])):
        s += voting_dict[sen_a][i] * voting_dict[sen_b][i]
    return s
...
def most_similar(sen, voting_dict):
    ...
    """
    return max([(policy_compare(sen, x, voting_dict), x) for x in voting_dict if x != sen])[1]
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'b'
    """
    return min([(policy_compare(sen, x, voting_dict), x) for x in voting_dict if x != sen])[1]
...
most_average_Democrat = 'Bayh' # give the last name (or code that computes the last name)
...
def find_average_record(sen_set, voting_dict):
    ...
    return [sum(vote) / len(vote) for vote in zip(*votes)]

voting_data = list(open("voting_record_dump109.txt"))
demograts = {y[0] for y in [x.split() for x in voting_data] if y[1] == 'D'}
average_Democrat_record = find_average_record(demograts, create_voting_dict(voting_data)) # give the vector as a list
...
def bitter_rivals(voting_dict):
    ...
    for sen1 in voting_dict:
        for sen2 in voting_dict:
            if sen1 != sen2:
                diff = policy_compare(sen1, sen2, voting_dict)
                if abs(diff) > maxDiff:
                    maxDiff = abs(diff)
                    name = (sen1, sen2)                
    return name
